...
def SimpleGNCNN(num_input_channels: int, features_dim: int) -> nn.Module:
...
def SimpleGNCNN2(num_input_channels: int, features_dim: int) -> nn.Module:
    """A simplified feature extractor with GroupNorm.
    ...
    model = nn.Sequential(
        ...
        nn.Flatten(),
        nn.Linear(in_features=1568, out_features=features_dim),
        # nn.Linear(in_features=6272, out_features=features_dim),
    )
