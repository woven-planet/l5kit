...
from l5kit.environment.benchmark import L5DatasetCacheEnv, L5RasterBaseEnv, L5RasterCacheEnv
...
def rollout(env: VecEnv, n_envs: int, total_eps: int = 10, total_steps: int = 10000,
    ...
    while True:
        _, rewards, dones, info = env.step(dummy_act)
        num_steps += n_envs
        if n_envs == 0:
            dones = [dones]
...
if __name__ == "__main__":
    ...
    if cfg["gym_params"]["pre_compute_rasters"]:
        if os.path.isfile('raster_dict.pkl'):
