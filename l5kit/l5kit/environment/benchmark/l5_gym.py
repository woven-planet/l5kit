""" Benchmark Complete L5Kit Gym Environment """
...
from l5kit.environment.benchmark import L5Env
from l5kit.environment.cle_utils import aggregate_cle_metrics, get_cle, SimulationOutputGym
from l5kit.rasterization import build_rasterizer


def get_dummy_obs(n_envs: int) -> Dict[str, np.ndarray]:
    """This function outputs dummy observation for the trained ego model.

    :param n_envs: the number of parallel gym envts
    :return: the dummy observation
    """
    print("Warning: Dummy Observation being used for the Ego Model for IPC Ablation")
...
def rollout(env: VecEnv, n_envs: int, total_eps: int = 10, total_steps: int = 10000,
    ...
    # IPC Ablation
    ipc_ablation = env.get_attr("raster_size") != env.get_attr("raster_out_size")
    if ipc_ablation:
        dummy_obs = get_dummy_obs(n_envs)
    ...
    if ego_model is not None:
        print("Using Model Actions")
        act = get_model_action(obs, ego_model, device, n_envs)
    else:
        print("Using Dummy Actions")
    ...
    while True:
        ...
        if sum(dones):
            if num_eps % 20 == 0:
                print(f"Rendered: {num_eps}/{total_eps} episodes")
            if n_envs == 0:
                # Need to manually reset when not using VecEnv
                obs = env.reset()
...
if __name__ == "__main__":
    ...
    use_ego_model = True if args.use_ego_model else cfg["gym_params"]["use_ego_model"]
    print("Use Ego Model: ", use_ego_model)
    if use_ego_model:
        assert cfg["model_params"]["history_num_frames"] == 5
        assert cfg["model_params"]["future_num_frames"] == 12
        assert cfg["raster_params"]["raster_size"][0] == 224
    ...
    env = L5Env(train_dataset, rasterizer, future_num_frames, num_simulation_steps,
    ...
    # DummyVecEnv
        ...
        env = make_vec_env(lambda: env, n_envs=n_envs)
    # No Multiprocessing. Only Main worker
    else:
