""" The L5Kit-based Gym environments """
...
class L5Env(gym.Env):
    ...
    def __init__(self, dataset: EgoDataset, rasterizer: Rasterizer, future_num_frames: int,
        ...
        self.action_space = spaces.Dict({'positions': spaces.Box(low=-100, high=100, shape=(1, future_num_frames, 2)),
                                         'yaws': spaces.Box(low=-100, high=100, shape=(1, future_num_frames, 1)),
                                         'velocities': spaces.Box(low=-100, high=100, shape=(1, future_num_frames, 2))})

        # Observation Space: gym.spaces.Dict (image: [n_channels, raster_size, raster_size])
        n_channels = rasterizer.num_channels()
        self.raster_size = rasterizer.raster_size[0]
    ...
    def step(self, action: gym.spaces.Dict) -> gym.spaces.Dict:
        ...
        if not self.sim_cfg.use_ego_gt:
            ...
            ego_frame_in_out = self.simulator.get_ego_in_out(self.ego_input_dict,
