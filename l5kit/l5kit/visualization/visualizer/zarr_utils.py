...
def simulation_ego_out_to_visualizer_scene(sim_out: SimulationOutput, mapAPI: MapAPI) -> List[FrameVisualization]:
    """Convert a simulation output into a scene we can visualize.
    The scene will include replayed and simulated trajectories for ego and agents when these are
    simulated.
    ...
    :return: a list of FrameVisualization for the scene
    """
    frames = sim_out.recorded_ego
    agents_frames = filter_agents_by_frames(frames, sim_out.recorded_agents)
    tls_frames = filter_tl_faces_by_frames(frames, sim_out.recorded_dataset.dataset.tl_faces)
    agents_th = sim_out.recorded_dataset.cfg["raster_params"]["filter_agents_threshold"]
    ego_ins_outs = sim_out.ego_ins_outs
    agents_ins_outs = sim_out.agents_ins_outs
...
def simulation_out_to_visualizer_scene_olt(sim_out: SimulationOutput, mapAPI: MapAPI) -> List[FrameVisualization]:
    """Convert a simulation output into a scene we can visualize.
    The scene will include replayed and simulated trajectories for ego and agents when these are
    simulated.
...
def simulation_out_to_visualizer_scene_clt(sim_out: SimulationOutput, mapAPI: MapAPI) -> List[FrameVisualization]:
    """Convert a simulation output into a scene we can visualize.
    The scene will include replayed and simulated trajectories for ego and agents when these are
    simulated.
