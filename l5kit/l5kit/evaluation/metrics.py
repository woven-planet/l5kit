...
def multi_trajectory_metric_single(gt: np.ndarray, pred: np.ndarray, confidences: np.ndarray) -> np.ndarray:
    """
    TODO legacy, remove
...
def multi_trajectory_metric(gt: np.ndarray, pred: np.ndarray, confidences: np.ndarray) -> np.ndarray:
    ...
    Args:
        ...
        confidences (np.ndarray): array of shape (batch)x(modes) with a confidence for each mode in each sample
    Returns:
        np.ndarray: array of shape (time,)
    ...
    gt = np.expand_dims(gt, 1)  # add modes
    confidences = np.expand_dims(confidences, -1)  # add time

    error = np.sum((gt - pred) ** 2, axis=-1)  # reduce coords
    error = confidences * np.exp(-0.5 * error)
    error = -np.log(np.sum(error, axis=-2))  # reduce modes
    error = np.mean(error, axis=0)  # reduce samples
    return error
