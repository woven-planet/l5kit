...
class MockModel(torch.nn.Module):
    ...
    def forward(self, x: Dict[str, np.ndarray]) -> np.ndarray:
        positions_and_yaws = torch.zeros(1, 1, 3)
...
def test_env_episode(dmg: LocalDataManager, env_cfg_path: str) -> None:
    ...
    for sim_output in sim_outputs:
        ego_tr = sim_output.simulated_ego["ego_translation"][: env.sim_cfg.num_simulation_steps, :2]
        ego_dist = np.linalg.norm(np.diff(ego_tr, axis=0), axis=-1)
        assert np.allclose(ego_dist, 1.0)

        ego_tr = sim_output.simulated_ego_states[: env.sim_cfg.num_simulation_steps,
        ...
        rots_sim = sim_output.simulated_ego["ego_rotation"][: env.sim_cfg.num_simulation_steps]
        ...
        # all rotations should be the same as the first one as the MockModel outputs 0 for that
        rots_sim = sim_output.simulated_ego_states[: env.sim_cfg.num_simulation_steps, TrajectoryStateIndices.THETA]
