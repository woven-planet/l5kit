...
class TestSatelliteImageCropping(unittest.TestCase):
    def test_satellite_image_cropping(self) -> None:
        ...
        for yaw in [None, 0.5 * np.pi, 0.25 * np.pi, 0.1234 * np.pi]:
            ...
            self.assertEqual(crop_im.shape, (200, 200, 3))
            self.assertAlmostEqual(crop_im[..., 0].mean(), 0.25, places=2)  # One quarter should still be red
            self.assertAlmostEqual(crop_im[..., 1].mean(), 0.25, places=2)  # One quarter should still be green
            if yaw == 0.5 * np.pi:
    ...
    def test_satellite_image_cropping_scaled(self) -> None:
        ...
        for yaw in [None, 0.321 * np.pi, 0.5 * np.pi, 1.0 * np.pi]:
            ...
            self.assertTrue(np.abs(crop_im.mean(axis=2).sum() - box_area_expected) < 5)  # Within 5 pixels error
