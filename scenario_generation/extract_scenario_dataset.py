...
from scenario_generation.helper_func import visualize_scene, get_poly_elems, agent_feat_dict_to_vec, is_valid_agent
from visualization_utils import visualize_scene_feat


####################################################################################
def process_scenes_data(scene_indices_all, dataset, dataset_zarr, dm, sim_cfg, cfg, min_n_agents, max_n_agents, min_extent_length,
    ...
    for i_scene, scene_idx in enumerate(scene_indices_all):
        ...
        if is_valid_agent(ego_speed, ego_extent, min_extent_length, min_extent_width):
        ...
        for (scene_id, agent_id) in agents_input.keys():  # if there are other agents besides the ego
            ...
            if is_valid_agent(speed, extent, min_extent_length, min_extent_width):
                agents_feat_dicts.append({
        ...
        for i_agent, i_agent_orig in enumerate(agents_dists_order):
            agents_feat_vecs[n_scenes, i_agent] = agent_feat_dict_to_vec(agents_feat_dicts[i_agent_orig],
